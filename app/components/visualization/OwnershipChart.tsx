    ...
    <div className="flex flex-col border rounded-lg bg-white">
      ...
      <div className="flex flex-col items-center gap-2 text-sm p-6 pt-0">
        <div className="flex items-center gap-2 leading-none font-medium">
          Total ownership:{" "}
          {chartData.reduce((sum, item) => sum + item.ownership, 0).toFixed(1)}
        ...
        <div className="text-muted-foreground leading-none text-center">
          <div>
            Founders:{" "}
            {founderEntries
              .reduce((sum, item) => sum + item.ownership, 0)
              .toFixed(1)}
            %
          </div>
          <div>
            Investors:{" "}
            {(
              investorEntries.reduce((sum, item) => sum + item.ownership, 0) +
              investorEntry.reduce((sum, item) => sum + item.ownership, 0)
            ).toFixed(1)}
            %
          </div>
